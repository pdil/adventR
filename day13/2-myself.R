...
for (j in 1:length(comb)) {
  ...
  total <- total + (df %>% filter(name == comb[[j]][1], next_to == comb[[j]][9]))$change
  ...
  total <- total + (df %>% filter(name == comb[[j]][9], next_to == comb[[j]][8]))$change
  total <- total + (df %>% filter(name == comb[[j]][9], next_to == comb[[j]][1]))$change
